                              ...
                              gestic_data_value_t mask, int timeout)
{
    ...
    return gestic_set_output_enable_mask(gestic, (gestic_data_mask_t) conv_flags[flags & 0x3F], (gestic_data_mask_t) 0,
                                         (gestic_data_mask_t) conv_flags[mask & 0x3F], timeout);
}

int gestic_set_stream_mask(gestic_t *gestic, gestic_data_value_t flags, int timeout) {
    return gestic_update_stream_mask(gestic, flags, (gestic_data_value_t) 0x3F, timeout);
}
