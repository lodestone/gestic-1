                               ...
                               const unsigned char *data)
{
    ...
    if(dataOutputConfig & gestic_DataOutConfigMask_DSPStatus) {
        ...
        if(calibration != 0) {
            dest->calib.reason = (gestic_calib_reason_t) calibration;
    ...
    if(dataOutputConfig & gestic_DataOutConfigMask_GestureInfo) {
        ...
        if(gesture) {
            dest->gesture.gesture = (gestic_gestures_t) gesture;
            dest->gesture.flags = (gestic_gesture_flags_t) (gestureInfo & gestic_gesture_flags_mask);
...
int gestic_data_stream_update(gestic_t *gestic, int *skipped) {
    ...
    if(count > 0) {
        ...
        if(gestic->result.gesture.last_event <= last_counter) {
            gestic->result.gesture.gesture = (gestic_gestures_t) 0;
            /* Reset flags except for the in-progress flag */
            gestic->result.gesture.flags = (gestic_gesture_flags_t) (gestic->result.gesture.flags & gestic_gesture_in_progress);
        }
        ...
        if(gestic->result.touch.last_tap_event <= last_counter)
            gestic->result.touch.tap_flags = (gestic_tap_flags_t) 0;
        ...
        if(gestic->result.calib.last_event <= last_counter)
            gestic->result.calib.reason = (gestic_calib_reason_t) 0;
